ScenarioB_up_eefun_min <- function(data, beta0range, contrast) {
  ...
  function(theta) {
    with(data, c( (1-Ytau)*(1-Z)*(Y-theta[1]),						# risk_0 = theta[1]
                  ...
                  ( theta[7] - (1- theta[2]/theta[6]) + (1- theta[4]/theta[5]) )*(contrast == "VE") +
                  ( theta[7] - (theta[2] - theta[6]) + (theta[4] - theta[5]) )*(contrast == "Difference") +
                  ( theta[7] - (log(theta[2]) - log(theta[6])) + (log(theta[4]) - log(theta[5])) )*(contrast == "logRR")
    ))
  }
  
}

ScenarioB_up_eefun_max <- function(data, beta0range, contrast) {
...
ScenarioB_up_eefun_cc_min <- function(data, beta0range, contrast) {
...
ScenarioB_up_eefun_cc_max <- function(data, beta0range, contrast) {
