# This code first produces Figure 5 of the manuscript Gilbert, Blette,
# Shepherd, Hudgens, which uses a VE contrast function
# h(x,y) = 1 - x/y  Secondly, the code produces Web Figure 7 of the
# manuscript, using almost identical code with the contrast option changed
# from contrast=VE to contrast=logRR.
...
# helper function for Imbens-Manski interval computation
...
# Make sure data are in working directory
data <- read.csv("HVTN505.csv")

data$R[is.na(data$R)] <- 0
Sstar_num <- c(7,6,8)   #Columns of data for each S variable
...
# Instead do the analysis using the 3 new binary markers suggested by the JASA reviewer
# Dec 2018

Sstar_num <- c(9,10,11)   #Columns of data for each S variable
...
# A new element of these analyses is that some people with R==1 have an NA value for the marker.
# Set the R values to 0 for such individuals.  But it is different sets for the three marker analyses.
# Delete the observations with R=1 but the marker is NA.  This is handled inside the analysis functions.
...
cat(paste("One i,j=",i,j),"\n")
    ...
    # Data to use for this biomarker
    keep <- data$R==0 | (data$R==1 & !is.na(data$S_star))
    data_widekeep <- count(data[keep,],c('Y','Z','Ytau','S_star','R'))
    data_widekeep$Group <- 1:nrow(data_widekeep)


    mats <- compute_matrices(list(eeFUN = low_fun,
                          splitdt = split(data_widekeep, f = data_widekeep$Group)),
                          numDeriv_options = list(method = 'simple'),
                          theta = thetahat_low_B, beta0range = c(-brange_list[j],brange_list[j]), contrast = "VE")

    A <- apply(simplify2array(Map(`*`, mats$A_i, data_widekeep$freq)), 1:2, sum)
    B <- apply(simplify2array(Map(`*`, mats$B_i, data_widekeep$freq)), 1:2, sum)
    ...
    if (j==0){
		...
		mats <- compute_matrices(list(eeFUN = up_fun,
                          splitdt = split(data_widekeep, f = data_widekeep$Group)),
                          numDeriv_options = list(method = 'simple'),
                          theta = thetahat_up_B, beta0range = c(-brange_list[j],brange_list[j]), contrast = "VE")
	  	A <- apply(simplify2array(Map(`*`, mats$A_i, data_widekeep$freq)), 1:2, sum)
	  	B <- apply(simplify2array(Map(`*`, mats$B_i, data_widekeep$freq)), 1:2, sum)
	  	Sigma <- solve(A) %*% B %*% t(solve(A))
    ...
    mats <- compute_matrices(list(eeFUN = low_fun,
                                  splitdt = split(data_widekeep, f = data_widekeep$Group)),
                             numDeriv_options = list(method = 'simple'),
                             theta = thetahat_low_C, beta0range = c(-brange_list[j],brange_list[j]), beta1range = c(-brange_list[j],brange_list[j]), contrast="VE")
    ...
    if (j==0){
      ...
      mats <- compute_matrices(list(eeFUN = up_fun,
                                    splitdt = split(data_widekeep, f = data_widekeep$Group)),
                               numDeriv_options = list(method = 'simple'),
                               theta = thetahat_up_C, beta0range = c(-brange_list[j],brange_list[j]), beta1range = c(-brange_list[j],brange_list[j]), contrast="VE")
      A <- apply(simplify2array(Map(`*`, mats$A_i, data_widekeep$freq)), 1:2, sum)
      B <- apply(simplify2array(Map(`*`, mats$B_i, data_widekeep$freq)), 1:2, sum)
      Sigma <- solve(A) %*% B %*% t(solve(A))
...
legend("topleft",legend=c("NEE-CB","NEB-CB"),col=c("black","dimgrey"),lty=1,cex=1,bty='n')
...
# Next produce Web Figure 7
# Re-do the analysis from scratch.
rm(list=ls())
...
# If contrast=="logRR", transform back to the 
# (1-VE(1))/(1-VE(0)) = RR(1)/RR(0) scale
...
# Basic descriptives for the Application section

kp <- data$Z==1 & data$Ytau==0 & !is.na(data$SstarAbCD8lowhi)
table(data$SstarAbCD8lowhi[kp],data$Y[kp])  # etc.
