...
up_eefun <- function(data, beta0range, beta1range, beta2range, beta3range,
  ...
  function(theta) {
    with(data, c( (1-Y_tau)*(1-Z)*(Y-theta[1]),												                    # risk_0 = theta[1]
              		...
              		( theta[19] - (1- theta[17]/theta[7]) + (1- theta[12]/theta[6]) )*(contrast == "VE") +
              		( theta[19] - (theta[17] - theta[7]) + (theta[12] - theta[6]) )*(contrast == "Difference") +
              		( theta[19] - (log(theta[17]) - log(theta[7])) + (log(theta[12]) - log(theta[6])) )*(contrast == "logRR") # CEP(1,0) - CEP(0,0) = theta[19]
